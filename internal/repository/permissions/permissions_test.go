...
func (r *PermRepoSuite) TestGetPermissionsForResource() {
	// fix time for testing
	testTime := time.Now()
	testError := errors.New("GetRoles error")
	...
	cases := []struct {
		name       string
		mockExpect func(mock sqlmock.Sqlmock)
		golden     []permissions.RoleAttributes
		wantErr    bool
	}{
		{
			name: "test success GetRoles",
			mockExpect: func(mock sqlmock.Sqlmock) {
				...
				mock.
					ExpectQuery(regexp.QuoteMeta(`SELECT * FROM "resources"  WHERE (resource_name = $1) ORDER BY "resources"."id" ASC LIMIT 1`)).
					WithArgs("user").
					WillReturnRows(sqlmock.NewRows([]string{"id", "created_at", "updated_at", "resource_name"}).
						AddRow(1, testTime, testTime, "user"))

				mock.
					ExpectQuery(regexp.QuoteMeta(`SELECT "roles".* FROM "roles" INNER JOIN "resource_role" ON "resource_role"."role_id" = "roles"."id" WHERE ("resource_role"."resource_id" IN ($1))`)).
					WithArgs(1).
					WillReturnRows(sqlmock.NewRows([]string{"id", "created_at", "updated_at", "name"}).
						AddRow(1, testTime, testTime, "admin").
						AddRow(2, testTime, testTime, "editor"))
				...
				mock.
					ExpectQuery(regexp.QuoteMeta(`SELECT * FROM "resource_role"  WHERE (resource_id = $1 AND role_id = $2) LIMIT 1`)).
					WithArgs(1, 1).
					WillReturnRows(sqlmock.NewRows([]string{"role_id", "resource_id", "can_create", "can_read", "can_update", "can_delete"}).
						AddRow(1, 1, true, true, true, true))

				mock.
					ExpectQuery(regexp.QuoteMeta(`SELECT * FROM "resource_role"  WHERE (resource_id = $1 AND role_id = $2) LIMIT 1`)).
					WithArgs(1, 2).
					WillReturnRows(sqlmock.NewRows([]string{"role_id", "resource_id", "can_create", "can_read", "can_update", "can_delete"}).
						AddRow(1, 1, true, true, true, false))
			},
			golden: []permissions.RoleAttributes{
				{
					RoleName:  "admin",
		...
		{
			name: "test fail GetRoles, fail to get resource",
			mockExpect: func(mock sqlmock.Sqlmock) {
				mock.MatchExpectationsInOrder(false)
				mock.
					ExpectQuery(regexp.QuoteMeta(`SELECT * FROM "resources"  WHERE (resource_name = $1) ORDER BY "resources"."id" ASC LIMIT 1`)).
		...
		{
			name: "test fail GetRoles, fail to get roles for resource",
			mockExpect: func(mock sqlmock.Sqlmock) {
				mock.MatchExpectationsInOrder(false)
				mock.
					ExpectQuery(regexp.QuoteMeta(`SELECT * FROM "resources"  WHERE (resource_name = $1) ORDER BY "resources"."id" ASC LIMIT 1`)).
		...
		{
			name: "test fail GetRoles, fail to get permissions for resource & 1st role",
			mockExpect: func(mock sqlmock.Sqlmock) {
				mock.MatchExpectationsInOrder(false)
				mock.
					ExpectQuery(regexp.QuoteMeta(`SELECT * FROM "resources"  WHERE (resource_name = $1) ORDER BY "resources"."id" ASC LIMIT 1`)).
		...
		{
			name: "test fail GetRoles, fail to get permissions for resource & 2nd role",
			mockExpect: func(mock sqlmock.Sqlmock) {
				mock.MatchExpectationsInOrder(false)
				mock.
					ExpectQuery(regexp.QuoteMeta(`SELECT * FROM "resources"  WHERE (resource_name = $1) ORDER BY "resources"."id" ASC LIMIT 1`)).
	...
	for _, tt := range cases {
		...
		r.T().Run(tc.name, func(t *testing.T) {
			...
			roles, err := r.repo.GetPermissionsForResource("user")
			if (err != nil) != tc.wantErr {
				t.Errorf("failed test to get roles. wantErr: %v, err: %v", tc.wantErr, err)
			}
